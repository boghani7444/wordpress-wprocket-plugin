...
function rocket_check_key( $type = 'transient_1', $data = null )
{
	...
	if ( ! rocket_valid_key()
		...
		$json = array();
		$json['consumer_key'] = WP_ROCKET_KEY;
		$json['consumer_email'] = WP_ROCKET_EMAIL;
		$json['secret_key'] = hash( 'crc32', WP_ROCKET_EMAIL );
        $json['success'] = true;
		$json['reason'] = '';
		...
		if ( $json ) {
			...
			$rocket_options['consumer_key'] 	= $json['consumer_key'];
			$rocket_options['consumer_email']	= $json['consumer_email'];

			if( $json['success'] ) {

				$rocket_options['secret_key'] = $json['secret_key'];
				...
				add_settings_error( 'general', 'settings_updated', $messages[ $json['reason'] ], 'error' );
